...
- `extract(int& task)`- извлекает задачу из очереди, если задач нет, то поток засыпает через condition_variable
- `stop_all` - активирует ожидающие потоки для завершения работы программы


**Функция void thread_func(int id, TaskQueue& q)**
- Принимает id потока и ссылку на очередь
...
В функции thread_func в цикле while вызывается метод extract(task), и так как значение переменной full\_stop равно true и очередь в этот момент пуста, то цикл завершается

Далее главный поток ожидает окончания выполнения рабочих потоков через .join()
